const getReplitData = async ({ users }) => {
  ...
  `,
    ...
    try {
      ...
      userData.userInfo = {
        average_rating: stats["averageHunterRating"],
        completed_jobs: stats["completedBountiesCount"],
        reviews_count: stats["numHunterReviews"],
        total_earned: Math.floor(
          hunterEarnings["approximateCyclesEarned"] / 100,
        ),
      };
    } catch (error) {
      ...
      const completedWithReviewsBounties = bountiesListings
        ...
        .map((listing) => {
          ...
          return {
            ...
            avatar_url: poster.image,
            job_link: `https://replit.com/bounties/@${poster.username}/${listing.slug}`,
...
const getTestimonialsData = async (bountyHunters) => {
  const replitData = await getReplitData({
    users: bountyHunters,
  });
